...
**字面量：**1，1.121这种在程序中的固定值称为字面量，整型字面量默认为int，浮点型字面量默认为为double。
...
short s1 = 2;
char c1 = 3;
b1 + s1 + c1;    // 结果为多少？结果的数据类型是什么?
```

```java
1 + 2 + 3        // 结果为多少？结果的数据类型是什么?
1.0 + 2 + 3      // 结果为多少？结果的数据类型是什么?
```
...
5、被fianl修饰的变量不会自动改变类型，如果所有的操作数都被final修饰，结果会根据左边变量的类型而转化。
