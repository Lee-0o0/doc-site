...
![image-20220910104532731](img/08.常用类/image-20220910104532731.png)
...
- 拆箱（Unboxing）：编译器自动将包装类型转换为对应的基本数据类型，这称为拆箱。比如，将Integer转换为int
...
|  6   |      public static double pow(double a, double b)       |           指数函数           |
|  7   |           public static double log(double a)            |    对数函数（以E为底数）     |
|  8   |          public static double log10(double a)           |    对数函数（以10为底数）    |
|  9   |           public static double sqrt(double a)           |            开平方            |
|  10  |              public static double random()              | 返回一个在[0, 1)之间的随机数 |
...
- Math.round(double)：我们通常会说这个方法表示”四舍五入”，但是当参数为负数时，就不太好理解。 所以，以源码的计算方式来理解会比较准确：即将原来的数字加上0.5后再向下取整。
...
- Math.rint(double)：返回最接近参数的整数，如果有2个数同样接近，则会返回偶数的那个。
